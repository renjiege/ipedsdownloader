...
survey_name = "Institutional Characteristics"
key_word = "Directory information"
start_year = 1980
end_year = 2016
...
def get_row(key_word):
    title = driver.find_elements_by_css_selector("#contentPlaceHolder_tblResult > tbody > tr > td:nth-of-type(3)")  # all links in column 5
    row = 1  # used to locate the stata do file (row number)
    for each in title:
        row += 1
        data_url = each.text
        if re.search(key_word, data_url, re.IGNORECASE):
            return str(row)

def get_data_url(row):
    data = driver.find_element_by_css_selector("tbody > tr:nth-of-type(%s) > td:nth-of-type(5) > a" % row)  # all links in column 5
    data_url = data.get_attribute("href")
    return data_url
...
def store_urls(urls):
    ...
    with open(data_path + key_word + ".txt", 'w') as file:
        json.dump(urls, file)
...
def get_urls(start, end, survey = None):
    ...
    for year in range(start, end):
        try:
            query(year, survey)
            if year == 1980 or 1984 or 1985 or 1996 or 2006:
                row = str(2)
            else:
                row = get_row(key_word)
            data_url = get_data_url(row)
...
def download_data():
    with open(data_path + key_word + ".txt", 'r') as file:
...
def main():
    if not os.path.isfile(data_path + key_word + ".txt"):
...
def rewrite_do_file():
    ...
    for file in os.listdir(data_path):
        if file.endswith(".do"):
            print("do \"" + file + "\"")
            with open(data_path+file, 'r', encoding='utf-8', errors='ignore') as openfile:
...
def rename_data_files():
    for filename in os.listdir(data_path+survey_name):
        if filename.endswith(".dta"):
            ...
            os.rename(filename, "hd"+str(year)+".dta")
