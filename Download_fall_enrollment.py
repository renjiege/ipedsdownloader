...
def get_data_url():
    ...
    for each in data_links:
        ...
        if re.search(r"EF\d\d(\d\d)?(_)?(A|(ANR))?_Data_Stata", data_url, re.IGNORECASE):
...
def store_urls(urls):
    ...
    with open(data_path + "urls.txt", 'w') as file:
        json.dump(urls, file)
...
def download_data():
    with open(data_path + "urls.txt", 'r') as file:
...
def main():
    if not os.path.isfile(data_path + "urls.txt"):
...
def rewrite_do_file():
    if not os.path.exists(data_path + "/Fall_Enrollment/"):
        os.mkdir(data_path + "/Fall_Enrollment/")
    for file in os.listdir(data_path):
        if file.endswith(".do"):
            ...
            with open(data_path+"/Fall_Enrollment/"+file, 'w') as newfile:
...
def rename_data_files():
    for filename in os.listdir(data_path + survey_name):
        if filename.endswith(".dta"):
            ...
            os.rename(filename, "ef" + str(year) + ".dta")
